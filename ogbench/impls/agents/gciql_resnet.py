...
from utils.networks import GCResnetActor, GCResnetValue

class GCIQLResnetAgent(flax.struct.PyTreeNode):
    ...
    def create(
        ...
        value_def = GCResnetValue(
            d_model=config['resnet_d_model'],
            num_layers=config['num_resnet_layers'],
            ensemble=False,
            gc_encoder=encoders.get('value'),
        )

        if config['discrete']:
            raise NotImplementedError("Discrete critic is not implemented for GCIQLResnetAgent.")
        else:
            critic_def = GCResnetValue(
                d_model=config['resnet_d_model'],
                num_layers=config['num_resnet_layers'],
                ensemble=True,
        ...
        if config['discrete']:
            raise NotImplementedError("Discrete actor is not implemented for GCIQLResnetAgent.")
        else:
            actor_def = GCResnetActor(
                d_model=config['resnet_d_model'],
                num_layers=config['num_resnet_layers'],
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            agent_name='gciql_resnet',  # Agent name.
            lr=3e-4,  # Learning rate.
            batch_size=1024,  # Batch size.
            num_resnet_layers=5,  # Number of layers in the ResNet.
            resnet_d_model=256,  # Dimension of the ResNet.
