...
from utils.networks import GCInterpActor, GCInterpValue
    
class GCIQLInterpAgent(flax.struct.PyTreeNode):
    ...
    def create(
        ...
        value_def = GCInterpValue(
            d_model=config['interp_d_model'],
            thinking_steps=config['num_interp_thinking_steps'],
            num_layers=config['num_interp_layers'],
            ensemble=False,
            gc_encoder=encoders.get('value'),
        )

        if config['discrete']:
            raise NotImplementedError("Discrete critic is not implemented for GCIQLInterpAgent.")
        else:
            critic_def = GCInterpValue(
                d_model=config['interp_d_model'],
                thinking_steps=config['num_interp_thinking_steps'],
                num_layers=config['num_interp_layers'],
                ensemble=True,
        ...
        if config['discrete']:
            raise NotImplementedError("Discrete actor is not implemented for GCIQLInterpAgent.")
        else:
            actor_def = GCInterpActor(
                d_model=config['interp_d_model'],
                thinking_steps=config['num_interp_thinking_steps'],
                num_layers=config['num_interp_layers'],
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            agent_name='gciql_interp',  # Agent name.
            lr=3e-4,  # Learning rate.
            batch_size=1024,  # Batch size.
            num_interp_layers=1,  # Number of layers in the IQL interpreter.
            interp_d_model=256,  # Dimension of the IQL interpreter.
            num_interp_thinking_steps=3,  # Number of thinking steps in the IQL
