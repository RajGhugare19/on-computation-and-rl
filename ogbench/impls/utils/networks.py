...
class GCResnetActor(nn.Module):
    ...
    def setup(self):
        ...
        self.resnet_layers = [
            ResidualBlock(use_ln=self.layer_norm)
    ...
    def __call__(
        ...
        x = self.input_embed_layer(inputs)
        for layer in self.resnet_layers:
            x = layer(x)
        final_out = x

        means = self.mean_net(final_out)
        if self.state_dependent_std:
            log_stds = self.log_std_net(final_out)
        else:
            if self.const_std:
                log_stds = jnp.zeros_like(means)
            else:
...
class ResnetValue(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        q_value = self.value_net(final_out)
        
        return q_value

class GCResnetValue(nn.Module):
    d_model: int
    num_layers: int
    ensemble: bool = True
    gc_encoder: nn.Module = None
    ...
    def setup(self):
        critic_cls = ResnetValue
        if self.ensemble:
            critic_cls = ensemblize(critic_cls, 2)
        
        self.value_net = critic_cls(
...
class GCInterpActor(nn.Module):
    ...
    def __call__(
        ...
        batch_dims = x_emb.shape[:-1]        
        c = jnp.zeros((*batch_dims, self.d_model))
        h = jnp.zeros((*batch_dims, self.d_model))
